...
(async () => {
  ...
  await drawer.locator('.ms-bug-detail-section-header .el-button').first().click({ force: true });
  await page.waitForTimeout(500);
  const actionButtons = await drawer.locator('.ms-bug-inline-actions .el-button').allInnerTexts();
  const editorVisible = await drawer.locator('.bug-rich-editor').isVisible();
  console.log(JSON.stringify({ actionButtons, editorVisible, messages }, null, 2));
  await context.close();
})();
